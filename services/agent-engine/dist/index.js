...
app.post('/chat', async (request, reply) => {
    ...
    const effectiveTraceId = traceId || (0, crypto_1.randomUUID)();
...
async function processChatInline(userMessageText, traceId) {
    ...
    if (availableTools.some((t) => t.name === 'github_pr_review')) {
    ...
    if (availableTools.some((t) => t.name === 'send_email')) {
        toolDeclarations.push({
            name: 'send_email',
    ...
    if (response.functionCalls &&
        ...
        textResp = firstPart?.text || "[Empty response]";
    }
    const t1 = Date.now();
...
function handleToolCall(call, agent, traceId, apiCost, tokensUsed) {
